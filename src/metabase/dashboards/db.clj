...
(mu/defn insert-dashboard-tabs!
  ...
  [rows :- [:sequential
            (mut/select-keys ::dashboards.schema/dashboard-tab.update [:dashboard_id :name :position :entity_id :is_shown])]]
...
(mu/defn update-dashboard-tab!
  ...
  [tab-id  :- ms/PositiveInt
   changes :- (mut/select-keys ::dashboards.schema/dashboard-tab.update [:name :position :is_shown])]
